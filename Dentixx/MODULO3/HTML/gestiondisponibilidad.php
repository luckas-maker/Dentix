...
<body class="h-full bg-gradient-to-br from-blue-50 to-indigo-100 font-sans">
            ...
            <div class="flex justify-between items-center">
                <div class="flex items-center space-x-4">
                    ...
                    <nav class="hidden md:flex space-x-4 ml-8">
                        <button onclick="window.location.href='../../MODULO3/HTML/agenda.php'" class="text-white font-bold hover:text-gray-200 transition-colors">Agendar citas</button>
                        <button onclick="window.location.href='../../MODULO3/HTML/citasolicitadas.php'" class="text-white font-bold hover:text-gray-200 transition-colors">Citas solicitadas</button>
                        <button onclick="window.location.href='../../MODULO3/HTML/gestiondisponibilidad.php'" class="text-white font-bold hover:text-gray-200 transition-colors">Gestion de disponibiliudad</button>
                        <button onclick="window.location.href='../../MODULO3/HTML/historial.php'" class="text-white font-bold hover:text-gray-200 transition-colors">Historial de citas</button>
                    </nav>
    ...
    <script>
        ...
        async function editSlotTime(slotId, currentTime) { // editar hora de inicio
            const newTime = prompt('Ingrese el nuevo horario (formato HH:MM):', currentTime);
            ...
            if (newTime && /^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$/.test(newTime)) {
                ...
                try {
                    ...
                    const result = await response.json();
                    if (result.success) {
                        // recargar data para actualizar información
                        await loadAvailabilityData(selectedDateStr);
                        displayTimeSlots();
                        generateCalendar();
                    } else {
                        alert('Error al actualizar el horario. Puede que ya exista un horario con esa hora.');
                    }
                } catch (error) {
                ...
                alert('Formato de hora inválido. Use HH:MM (ejemplo: 09:30)');
            }
        }
        ...
        async function addCustomSlot() {
            ...
            if (newTime && /^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$/.test(newTime)) {
                try {
                    ...
                    const result = await response.json();
                    if (result.success) {
                        // recargar datos disponibles
                        await loadAvailabilityData(selectedDateStr);
                        displayTimeSlots();
                        generateCalendar();
                    } else {
                        alert('Ya existe un horario con esa hora o ocurrió un error');
                    }
                } catch (error) {
        ...
        async function clearAllSlots() { //eliminar todos los datos
            ...
            if (confirm('¿Está seguro de que desea eliminar todos los horarios de esta fecha?')) {
