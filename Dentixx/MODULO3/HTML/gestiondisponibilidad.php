    ...
    <script>
        ...
        function generateCalendar() { // genera un calendario estatico
            ...
            for (let i = 0; i < 42; i++) {
                ...
                if (isPast) {
                    // días anteriores deshabilitados
                    dayElement.className += ' opacity-40 cursor-not-allowed';
                    dayElement.title = 'No puede seleccionar una fecha pasada';
                    dayElement.onclick = () => showSuccessModal('Por favor seleccione una fecha válida (hoy o días futuros).', 'error');
                } else {
    ...
    if (success) {
        ...
        async function editSlotTime(slotId, currentTime) { // editar hora de inicio
            showEditHorarioModal(currentTime, async (newTime) => {
                ...
                // Modal bonito para editar horario
        ...
        async function addCustomSlot() {
            ...
            showAddHorarioModal(async (newTime) => {
                ...
                // Modal bonito para ingresar horario
