...
<!-- Logo centrado cuadrado para vista móvil -->
<div class="flex justify-center mb-6 lg:hidden">
    <img src="../../assets/img/logo-writopeks.jpg" alt="Writo Peks Consultorio Dental"
         class="w-24 h-24 object-cover border-4 border-white shadow-lg rounded-xl" />
</div>

<!-- Logo centrado cuadrado para vista escritorio -->
...
<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div id="apellidos-error" class="hidden text-sm text-red-600 mt-2 flex items-center">
            ...
            Por favor ingresa solo letras.
                ...
                <div>
                    ...
                    <input type="email" id="registro-email" required oninput="resetVerificacionEmail()"
                        class="input-modern w-full px-4 py-3 border-2 border-gray-200 rounded-xl shadow-sm focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500"
                        placeholder="tu@email.com">
                    
                    <button type="button" id="validar-email-btn" onclick="iniciarVerificacionEmail()" class="text-sm font-semibold dental-accent hover:text-blue-800 transition-colors mt-2 inline-flex items-center">
                        <i class="fas fa-shield-alt mr-1"></i>
                        Validar correo electrónico
                    </button>
                    
                    <div id="email-verificado-success" class="hidden mt-2 text-sm text-green-700 font-semibold flex items-center bg-green-100 p-2 rounded-lg">
                    ...
                    <div id="email-no-verificado-error" class="hidden text-sm text-red-600 mt-2 flex items-center">
                        <i class="fas fa-exclamation-circle mr-1"></i>
                        Debes verificar tu correo antes de registrarte.
                    </div>
                </div>
    ...
    <div id="verificar-email-modal" class="modal">
        <div class="modal-content">
            ...
            <p class="text-gray-600 mb-6 text-sm sm:text-base">
                ...
                <strong id="email-a-verificar" class="text-gray-800"></strong>.
                <br>
                <span class="text-xs text-gray-500">(Para esta demo, el código es: <strong>123456</strong>)</span>
            </p>
    ...
    <script>
        ...
        // NUEVO: Variables para la verificación de registro
        ...
        function enviarCodigoRecuperacion(event) {
            ...
            mostrarAlerta("success", "Código de verificación enviado a tu correo.", "alert-recuperacion");
            setTimeout(() => {
                ocultarRecuperacion();
                mostrarVerificacion();
            }, 1500);
        ...
        function verificarCodigo(event) {
            ...
            mostrarAlerta("success", "Código verificado correctamente.", "alert-verificar");
            setTimeout(() => {
                ocultarVerificacion();
                mostrarCambio();
            }, 1500);
        ...
        // NUEVAS FUNCIONES PARA LA VALIDACION DEL CORREO ELECTRONICO

        async function iniciarVerificacionEmail() {
    const emailInput = document.getElementById("registro-email");
    const email = emailInput.value;
    const emailError = document.getElementById("registro-email-error");
    const emailExistenteError = document.getElementById("email-existente-error");
    ...
    console.log("Paso 1: Email válido:", email);

    // 2. Verificar si el email ya existe en la BD
    try {
        console.log("Paso 2: Verificando email en BD...");
        ...
        console.log("Respuesta check-email status:", response.status);
        const data = await response.json();
        console.log("Respuesta check-email data:", data);

        if (!data.available) {
            emailExistenteError.classList.remove("hidden");
            return;
        }
        emailExistenteError.classList.add("hidden");

        console.log("Paso 3: Email disponible, enviando código...");

        // 3. Si todo OK, enviar código y mostrar modal de verificación
        const sendResponse = await fetch('../../api/send-verification.php', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
        ...
        console.log("Respuesta send-verification status:", sendResponse.status);
        const sendData = await sendResponse.json();
        console.log("Respuesta send-verification data:", sendData);

        if (sendData.success) {
            emailEnVerificacion = email;
            tokenRegistro = sendData.token; // Para demo
            document.getElementById("email-a-verificar").innerText = emailEnVerificacion;
            document.getElementById("verificar-email-modal").style.display = "flex";
            mostrarAlerta("success", "Código enviado a tu correo.", "alert-verificar-email");
        } else {
            mostrarAlerta("error", sendData.message, "alert-registro");
        }
    } catch (error) {
        console.error("Error completo:", error);
        mostrarAlerta("error", "Error de conexión: " + error.message, "alert-registro");
    }
}
    ...
    const errorDiv = document.getElementById("codigo-email-error");
    ...
    // Verificar código (en demo comparamos directamente)
    if (codigo === tokenRegistro) {
        emailVerificado = true;
        mostrarAlerta("success", "Correo verificado exitosamente.", "alert-verificar-email");
        
        setTimeout(() => {
            ocultarVerificacionEmail();
            bloquearCampoEmail(); // Función dedicada para bloquear el email
        }, 1500);
...
function bloquearCampoEmail() {
    const emailInput = document.getElementById("registro-email");
    const validarBtn = document.getElementById("validar-email-btn");
    const successDiv = document.getElementById("email-verificado-success");
    ...
    emailInput.style.borderColor = "#10b981"; // Verde para indicar verificación
    emailInput.style.boxShadow = "0 0 0 2px rgba(16, 185, 129, 0.1)";
    
    console.log("Campo de email bloqueado - verificación exitosa");
}

        // FUNCION AGREGADA: Resetear verificación de email
function resetVerificacionEmail() {
    if (!document.getElementById("registro-email").readOnly) {
        emailVerificado = false;
        emailEnVerificacion = "";
        tokenRegistro = "";
        document.getElementById("validar-email-btn").classList.remove("hidden");
        document.getElementById("email-verificado-success").classList.add("hidden");
        document.getElementById("email-no-verificado-error").classList.add("hidden");
    } else {
        // Si está bloqueado, permitir desbloquearlo solo si el usuario confirma
        if (confirm("¿Estás seguro de que quieres cambiar el correo verificado? Perderás la verificación actual.")) {
            desbloquearCampoEmail();
        }
    }
}
...
function desbloquearCampoEmail() {
    ...
    console.log("Campo de email desbloqueado");
}
    ...
    if (!regexNombre.test(nombres) || nombres.length < 2) {
        document.getElementById("nombres-error").classList.remove("hidden");
        valid = false;
    } else {
        document.getElementById("nombres-error").classList.add("hidden");
    }
    // Validar apellidos
    if (!regexNombre.test(apellidos) || apellidos.length < 2) {
        document.getElementById("apellidos-error").classList.remove("hidden");
        valid = false;
    } else {
        document.getElementById("apellidos-error").classList.add("hidden");
    }
            ...
            if (!regexTelefono.test(telefono)) {
                document.getElementById("registro-telefono-error").classList.remove("hidden");
                valid = false;
            } else {
                document.getElementById("registro-telefono-error").classList.add("hidden");
            }

            // Verificar que el email haya sido verificado
            const errorNoVerificado = document.getElementById("email-no-verificado-error");
            if (!emailVerificado) {
                errorNoVerificado.classList.remove("hidden");
                valid = false;
            } else {
                errorNoVerificado.classList.add("hidden");
            }
    ...
    try {
        const response = await fetch('../../api/register.php', {
            ...
            body: JSON.stringify({ 
                ...
                if (data.success) {
                    mostrarAlerta("success", data.message, "alert-registro");
                    setTimeout(() => {
                        ocultarRegistro();
                        document.getElementById("registro-formulario").reset();
                        document.getElementById("registro-email").readOnly = false;
                        document.getElementById("registro-email").classList.remove("bg-gray-100", "text-gray-500");
                        resetVerificacionEmail();
                        // Mostrar mensaje en login
                        //mostrarAlerta("success", "Registro completado.", "alert-login");
                    }, 2000);
                } else {
...
async function validarLogin(event) {
            ...
            try {
                ...
                if (data.success) {
                    ...
                    setTimeout(() => {
                        //alert(`Bienvenido ${data.user.nombre}\nTipo de usuario: ${data.user.rol}`);
                        // Redirigir según el rol
                        if (data.user.rol === 'Odontologo') {
                            window.location.href = 'miperfil.php';
                        } else {
                            window.location.href = '../../MODULO2/HTML/veragendas.html';
                        }
                    }, 1500);
                } else {
        ...
        function ocultarVerificacionEmail() {
    document.getElementById("verificar-email-modal").style.display = "none";
}
