...
<script>
    ...
    async function enviarCodigoRecuperacion(event) {
        ...
        // (Fluidez) Deshabilitar botón y mostrar carga
        ...
        try {
            ...
            // (Fluidez) Reactivar botón
            button.disabled = false;
            button.innerHTML = '<i class="fas fa-paper-plane mr-2"></i>Enviar Código';
        }
        return false;
    }
    ...
    async function verificarCodigo(event) {
        ...
        try {
            ...
            button.disabled = false;
            button.innerHTML = '<i class="fas fa-check-circle mr-2"></i>Verificar Código';
        }
        return false;
    }
    ...
    async function cambiarContrasena(event) {
        ...
        if (nueva.length < 6) {
            ...
            valido = false;
        } else { document.getElementById("nueva-error").classList.add("hidden"); }
        
        if (nueva !== confirmar) {
            document.getElementById("confirmar-error").classList.remove("hidden");
            valido = false;
        } else { document.getElementById("confirmar-error").classList.add("hidden"); }
        ...
        try {
            ...
            button.disabled = false;
            button.innerHTML = '<i class="fas fa-save mr-2"></i>Cambiar Contraseña';
        }
        return false;
    }
    ...
    async function verificarCodigoEmail(event) {
        ...
        // 2. MODIFICACIÓN (FLUIDEZ): Deshabilitar botón y mostrar carga
        button.disabled = true;
        button.innerHTML = '<i class="fas fa-spinner fa-spin mr-2"></i>Accediendo...';
        
        // 3. MODIFICACIÓN: ELIMINADA la alerta "Verificando credenciales..."
        // mostrarAlerta("success", "Verificando credenciales...", "alert-login"); // <-- LÍNEA ELIMINADA

        try {
            // 4. RUTA CORREGIDA (debe subir dos niveles)
            ...
            if (data.success) {
                ...
                // ERROR: (Ej. "Cuenta pendiente", "Correo incorrecto")
                mostrarAlerta("error", data.message, "alert-login");
                
                // 5. MODIFICACIÓN (FLUIDEZ): Reactivar el botón si el login falla
                button.disabled = false;
            ...
            // 6. MODIFICACIÓN (FLUIDEZ): Reactivar el botón si hay un error de red
            button.disabled = false;
            button.innerHTML = '<i class="fas fa-sign-in-alt mr-2"></i>Iniciar Sesión';
        }
    ...
    // Si es un mensaje de "carga", no lo ocultes (se reemplazará por éxito/error)
    if (mensaje.includes("Procesando") || mensaje.includes("Verificando") || mensaje.includes("Enviando") || mensaje.includes("Accediendo")) {
        return; 
    }
    
    // Si es el error de cuenta pendiente (que es largo), dar más tiempo
